...
### 8. Codebase Navigation & Token Optimization (Graphify)
- **Graph-First Policy:** A structural knowledge graph exists for this project in `graphify-out/graph.json`. 
- **Mandatory Tool Use:** Before using `grep`, `glob`, or reading raw source files to understand architecture, dependencies, or find function definitions, you MUST consult the graph using the `/graphify` skill (e.g., `/graphify query ...`).
- **Token Limits:** Only open specific raw `.py` files after identifying the exact targets via the graph. Do not scan whole directories.