...
saturating $\tanh$ bottleneck, and the resulting model is roughly an order of
magnitude worse in MAE; the near-linear ``next $\approx$ last'' signal that
dominates these series is destroyed by the recurrent nonlinearity. Because
...
\input{paper_assets/algorithms/algo_causal_stgnn.tex}
